        ...
        <div class="col-12">
            <div class="card">
                ...
                >
                    <template #header>
                        ...
                        <div class="flex justify-content-between gap-6 mt-2" :style="{ 'margin-left': '-15px' }">
                            ...
                            <div class="flex gap-2" :style="{ 'margin-right': '-22px' }">
                                ...
                                <Button type="button" label="New Coupon" icon="pi pi-plus" :style="{ 'background-color': 'darkgreen', border: 'darkgreen' }" @click="openAddNewFlashSale()"></Button>
                            </div>
                        </div>
                    </template>
                    ...
                    <Column field="startDate" header="Start Date" :sortable="true" headerStyle="width:14%; min-width:10rem;">
                        <template #body="slotProps">
                            <span class="p-column-title">Amount</span>
                            {{ slotProps.data.category }}
                        </template>
                    </Column>
                    <Column field="endDate" header="End Date" :sortable="true" headerStyle="width:14%; min-width:10rem;">
                        <template #body="slotProps">
                ...
                <Dialog v-model:visible="addFlashSale" :style="{ width: '450px' }" :modal="true" class="p-fluid">
                    <template #header>
                        <div class="p-d-flex p-ai-center">
                            ...
                            <span class="p-ml-n6 ml-2" style="color: darkgreen; font-weight: 700; font-size: larger">Add Coupon</span>
                            <!-- PrimeFlex classes for margin-left and bold text -->
                        </div>
                    </template>
                    <div class="flex justify-content-between gap-2">
                        <div>
                            <label for="name" :style="{ 'font-weight': 'bold', 'font-size': 'small' }">Coupon Code</label>
                            <InputText id="username2" type="username" class="p-error mt-1" aria-describedby="username-help" />
                        </div>
                    ...
                    <div class="field mt-3">
                        ...
                        <Dropdown id="state" v-model="status" :options="statuses" optionLabel="value" placeholder="Select Coupon"></Dropdown>
                    </div>
                    <div class="field">
                        <label for="name" :style="{ 'font-weight': 'bold', 'font-size': 'small' }">Customer</label>
                        <Dropdown  id="state" v-model="status" :options="statuses" optionLabel="value" placeholder="Select Customer"></Dropdown>
                    </div>
                    <div class="flex justify-content-between gap-2">
                        <div>
                            <label for="name" :style="{ 'font-weight': 'bold', 'font-size': 'small' }">Discount Type</label>
                            <Dropdown id="state" v-model="status" :options="statuses" optionLabel="value" placeholder="Percentage(%)" class="mt-1"></Dropdown>
                        </div>
                        <div>
                            <label for="description" :style="{ 'font-weight': 'bold', 'font-size': 'small' }">Coupon Amount</label>
                            <InputNumber class="mt-1" id="price" v-model="product.price" mode="currency" currency="USD" locale="en-US" :invalid="submitted && !product.price" :required="true" />
                        </div>
                    </div>
                    <div class="field mt-3">
                        <label for="name" :style="{ 'font-weight': 'bold', 'font-size': 'small' }">User Limit</label>
                        <Dropdown id="state" class="mt-1" v-model="status" :options="statuses" optionLabel="value" default="0"></Dropdown>
                    </div>
                    <div class="flex justify-content-between mt-3 gap-2">
                        <div>
                            ...
                            <Calendar class="mt-1" v-model="calenderValue" :style="{ borderRadius: '8px' }" :showIcon="true" :manualInput="false" placeholder="DD/MM/YYYY"></Calendar>
                        </div>
                        <div>
                            <label for="orderDate" :style="{ 'font-weight': 'bold', 'font-size': 'small' }">Expiry Date</label>
                            <Calendar class="mt-1" v-model="calenderValue" :style="{ borderRadius: '8px' }" :showIcon="true" :manualInput="false" placeholder="DD/MM/YYYY"></Calendar>
                        </div>
                    </div>
                    <template #footer>
                        ...
                        <Button type="button" label="Add Coupon" icon="pi pi-plus" :style="{ 'background-color': '#1E4A35', border: '#1E4A35', color: 'white' }" @click="saveProduct()"></Button>
                    </template>
                </Dialog>
                ...
                <Dialog v-model:visible="editFlashSaleDialog" :style="{ width: '450px' }" :modal="true" class="p-fluid">
                    <template #header>
                        <div class="p-d-flex p-ai-center">
                            <span class="pi pi-plus p-mr-1" style="color: darkgreen; font-weight: 700"></span>
                            <!-- PrimeIcons class for the filter icon with custom color -->
                            <span class="p-ml-n6 ml-2" style="color: darkgreen; font-weight: 700; font-size: larger">Edit Coupon</span>
                            <!-- PrimeFlex classes for margin-left and bold text -->
                        </div>
                    </template>
                    <div class="flex justify-content-between gap-2">
                    ...
                    <template #footer>
                        ...
                        <Button type="button" label="Save Coupon" icon="pi pi-save" :style="{ 'background-color': 'darkgreen', border: 'darkgreen' }" @click="saveProduct()"></Button>
                    </template>
                </Dialog>
                ...
                <Dialog v-model:visible="deleteProductDialog" :style="{ width: '450px' }" :closable="false" :modal="true">
                    <template #header>
                        <div>
                            ...
                            <div class="mt-1">
                                <span :style="{ color: gray }">This action cannot be undone. This will permanently remove the coupon.</span>
                            </div>
                        </div>
                    </template>
                    <template #footer>
